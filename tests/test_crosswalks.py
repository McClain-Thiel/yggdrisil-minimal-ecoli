...
def test_crosswalks_preserve_the_gene_universe_and_report_mapping_gaps() -> None:
    registry = parse_ncbi_gff(FIXTURES / "mg1655_excerpt.gff3")[0]
    ...
    assert mapped.index.equals(registry.index)
    assert mapped.loc["b0001", "kegg_gene_id"] == "eco:b0001"
    assert mapped.loc["b0002", "ko_ids"] == ("K12524", "K99999")
    assert mapped.loc["b0003", "kegg_gene_id"] is None
    assert mapped.loc["b0002", "iml1515_gene_id"] == "b0002"
    assert mapped.loc["b0001", "iml1515_gene_id"] is None
    assert audit == {
